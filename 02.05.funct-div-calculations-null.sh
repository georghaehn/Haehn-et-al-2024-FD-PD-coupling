...
#SBATCH --job-name=FD-null-calculation
...
output_dir=/work/$USER/FD-null-calculation
mkdir -p "$output_dir"
...
Rscript --vanilla $HOME/sPlot-master-thesis/01.scripts/02.05.funct-div-calculations-null.R \
  "$input_dir" \
  "$output" \
  "$start" \
  "$end"
