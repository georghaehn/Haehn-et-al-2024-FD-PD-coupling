...
#SBATCH --job-name=PD-null-calculation
...
output_dir=/work/$USER/PD-null-calculation
mkdir -p "$output_dir"
...
Rscript --vanilla $HOME/sPlot-master-thesis/01.scripts/02.03.phylo-calculations-null.R \
  "$input_dir" \
  "$output" \
  "$start" \
  "$end"
