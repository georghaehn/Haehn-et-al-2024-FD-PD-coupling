...
sPlot.spa <- SpatialPointsDataFrame(
  coords = header %>%
...
chelsa.world <- as.data.frame(chelsa.world) %>%
  rename_at(
    .vars = vars(starts_with("CHELSA")),
    .funs = ~ stringr::str_sub(., -8, -1)
  )

colnames(chelsa.world)[1:9] <- c(
  "bio10_1", "bio10_2", "bio10_3", "bio10_4", "bio10_5", "bio10_6",
...
(p1 <- ggplot(data = PCA.clim$x %>%
  ...
  geom_segment(
    data = vec.sp.df, aes(x = 0, xend = PC1, y = 0, yend = PC2),
    arrow = arrow(length = unit(0.1, "cm")), alpha = 0.9,
    colour = gray(0.1)
  ) +
  geom_text_repel(
    data = vec.sp.df, aes(x = PC1, y = PC2, label = Var), size = 10, segment.alpha = 1 / 3
...
sPlot.out <- sPlot.data %>%
  bind_cols(as.data.frame(PCA.clim$x) %>%
    ...
    mutate_at(
      .vars = vars(PC2), # flip PCA axes pointing to negative direction
      .funs = list(~ -.)
    ))
