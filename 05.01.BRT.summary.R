...
(p3 <- ggplot(
  data = BRT.sum %>% filter(Response == "log"),
  ...
  scale_fill_manual(
    labels = c(
      expression(paste("log(SES.FD"[Q] / "SES.PD"[Q], ")")),
      expression(SES.FD[Q]),
      expression(SES.PD[Q])
    ),
    values = c("darkcyan", "olivedrab3", "darkorchid4")
...
# ggsave(filename = "__Submission/Figures/04.BRT-summary.rel.png", width = 30, height = 20, units = "in", dpi = 600, p3)
(p3.0 <- ggplot(
  data = BRT.sum %>% filter(Response == "SES.FD"),
...
(p3.01 <- ggplot(
  data = BRT.sum %>% filter(Response == "SES.PD"),
...
ggsave(
  filename = "__Submission/Figures/05.BRT-summary-BW.png", width = 33, height = 10,
  units = "in", dpi = 600, ppp
)

ggsave(
  filename = "__Submission/Figures/Figure3.pdf", width = 33, height = 10,
  units = "in", dpi = 600, ppp
)
...
partial_plot <- function(x, vars) {
  if (length(vars) > 1) {
    ...
    (df <- partial_dependence(x, vars[[1]]))
  }
  df_mean <- df %>%
  ...
  ggplot2::ggplot(
    data = df %>%
      ...
      rbind(., df %>%
        filter(variable == "is.forest") %>%
        mutate(
          value = ifelse(value == 1, FALSE, TRUE),
