...
d.log <- dat %>%
  filter(
    quantile(rel.log.BW, 0.05) <= rel.log.BW,
    rel.log.BW <= quantile(rel.log.BW, 0.95)
  ) %>%
  group_by(cell) %>%
  summarise(
    Ratio = mean(rel.log.BW, na.rm = T),
    n = n()
  )
...
RQEP.RQEF.legend <- ggplotGrob(
  ggplot(d, aes(x, y, fill = atan(y / x), alpha = atan(x + y))) +
    ...
    labs(
      y = expression(paste(
        "SES Functional RQ Entropy",
        symbol("\256")
      )),
      x = expression(paste(
        "SES Phylogenetic RQ Entropy",
        symbol("\256")
      ))
    ) +
    theme_minimal() +
...
closest_colors <- mapply(color1 = out$color, alpha1 = out$alpha, function(color1, alpha1) {
  distances <- mapply(
    color2 = df.legend$fill, alpha2 = df.legend$alpha,
