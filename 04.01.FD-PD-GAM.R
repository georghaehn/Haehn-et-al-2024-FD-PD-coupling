...
# orig observed data
model1 <- mgcv::gam(
  RQE.MULTI ~ RaoD.phyl + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
model2 <- mgcv::gam(
  RQE.MULTI ~ RQEP.sqrt + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
model2.1 <- mgcv::gam(
  RQE.MULTI ~ 1 + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
model3 <- mgcv::gam(
  SES.RQEF ~ SES.RQEP + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
model3.1 <- mgcv::gam(
  SES.RQEF ~ 1 + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
model4 <- mgcv::gam(
  SES.RQEF ~ SES.RQEP.sqrt + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
modb <- mgcv::gam(
  SES.RQEF.B ~ SES.RQEP.B + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
modb1 <- mgcv::gam(
  SES.RQEF.B ~ 1 + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
modbw <- mgcv::gam(
  SES.RQEF.BW ~ SES.RQEP.BW + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
modbw.1 <- mgcv::gam(
  SES.RQEF.BW ~ 1 + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
modpk <- mgcv::gam(
  SES.RQEF.PK ~ SES.RQEP.PK + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
modpk.1 <- mgcv::gam(
  SES.RQEF.PK ~ 1 + s(Longitude, Latitude,
    bs = "sos"
  ),
  family = "gaussian",
  method = "REML",
...
(p.FPD <- ggplot() +
  ...
  scale_fill_gradient(
    high = "#0DE8EF", low = "#373C3D", trans = "log",
    limits = c(1, 100000),
...
(p.SES.sqrt <- ggplot() +
  ...
  labs(
    ...
    x = bquote(atop(
      "SES.PD"[Q] ~ "square root distances",
      "Global species pool"
    ))
  ) +
  theme_bw() +
...
(p.SES <- ggplot() +
  ...
  labs(
    y = expression(paste("Residuals SES.FD"[Q])),
...
p.RQE.SES <- ggplot() +
  ...
  stat_binhex(data = d.RQE.SES, aes(
    x = x, y = y,
    fill = ..count..
  )) +
  scale_fill_gradient(
...
(mid <- mtcars %>%
  ...
  # geom_point(aes(x = 1, y = -4), shape = 19, colour = "darkgreen", size = 10) +
  labs(
    title = expression(paste("Coupling with FD" %~~% "PD")),
...
pred.FD.PD.SES$cell <- dgGEO_to_SEQNUM(
  dggs,
  pred.FD.PD.SES$Longitude,
  pred.FD.PD.SES$Latitude
)$seqnum
...
bb <- ne_download(
  type = "wgs84_bounding_box", category = "physical",
  returnclass = "sf"
) %>%
  st_transform(crs = "+proj=eck4") %>%
  st_geometry()
...
pred.FD.PD$cell <- dgGEO_to_SEQNUM(
  dggs,
