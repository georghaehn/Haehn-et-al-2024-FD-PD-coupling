...
(p.phyl.PC1 <- ggplot() +
  ...
  labs(
    y = expression(paste("Residuals SES.PD"[Q])),
...
pred.RQEP.for <- lapply(forest.data, function(data) {
  predictions(mod.RQEP, newdata = datagrid(
...
(p.rel.PC1 <- ggplot() +
  ...
  labs(
    y = expression(paste("log(SES.FD"[Q] / "SES.PD"[Q], ")")),
    x = "PC1 - Annual precipitation\n(dry to wet)"
  ) +
  theme_bw() +
  theme(
...
pred.rel.for <- lapply(forest.data, function(data) {
  predictions(mod.rel, newdata = datagrid(
    PC1 = sample(size = 10, x = seq(min(sPlot.data$PC1), max(sPlot.data$PC1), .1), replace = T),
    is.forest = data$is.forest
  ))
})
...
pp.out <- wrap_elements(p.out.rel) +
  ...
  theme(
    plot.tag = element_text(
      size = 47, angle = 90, vjust = -6 # , hjust = 20
    ),
...
(p1 <- ll3[["PC1"]] |>
  filter(
    fitted < quantile(fitted, 0.95),
    fitted > quantile(fitted, 0.05)
  ) %>%
  ggplot(aes(x = PC1, y = fitted, group = rep)) +
  geom_rect(
    xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = min(ll3[["Thresh"]]$threshold),
    color = "darkorchid4", fill = NA, linewidth = 3
  ) +
  geom_rect(
    xmin = -Inf, xmax = Inf, ymin = min(ll3[["Thresh"]]$threshold) + 0.01,
    ymax = max(ll3[["Thresh"]]$threshold),
    color = "darkcyan", fill = NA, linewidth = 3
  ) +
  geom_rect(
    xmin = -Inf, xmax = Inf, ymin = max(ll3[["Thresh"]]$threshold) + 0.01,
