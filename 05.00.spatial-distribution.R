...
da <- expand.grid(x=seq(-0.4, 1.2, length.out = 1000),y=0)
log.leg <- ggplotGrob(
  ggplot(da, aes(x,y,fill=x))+
    geom_tile()+
    scale_fill_viridis() +
    scale_y_continuous(name = bquote(atop({SES.PD} [Q] ~">",
                                          {SES.FD} [Q])) ,
                       sec.axis = sec_axis(~./2, name = 
                                             bquote(atop({SES.FD} [Q] ~">",
                                                         {SES.PD} [Q]))
                                             
                                             ) )+
    scale_x_continuous(breaks = c(-0.25, 0, 0.25, 0.5, 0.75, 1))+
    theme_minimal() +
    theme(
      axis.title.y = element_text(size = 17, angle = 0),
      axis.title.y.right = element_text(size = 17, angle = 0),
...
(p.log <- grid %>% 
    ...
    annotation_custom(grob = log.leg,
                      # xmin =  -45e6,
                      # ymax =  -1.1e6,
                      # ymin =  -8.5e6
                      xmin = -9e6,
...
ggsave("__Submission/Figures/04.map-gam1.pdf", p.out, height=30, width=20, units="in", dpi=600, bg = "white")
