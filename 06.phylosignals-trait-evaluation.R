...
PD.fam <- tree.species %>% 
  ...
  group_by(family) %>%
  dplyr::summarize(pruned.tree=list(pruning(myspecies=species,
...
result <- foreach(j=1:length(traits)) %dopar% {
  ...
  for(i in 1:nrow(PD.fam)) {
    ...
    tryCatch({
      sig[[i]] <- data.frame(lambda = phytools::phylosig(mytree, trait, method = "lambda", test = T)$lambda,
                             family = PD.fam[i, "family"],
                             Response = traits[j])
    }, error = function(e) { sig[[i]] <- NA })
  }
...
result <- foreach(j=1:length(traits)) %dopar% {
  ...
  for(i in 1:nrow(PD.fam)) {
    ...
    tryCatch({
      sigK[[i]] <- data.frame(K = phytools::phylosig(mytree, trait, method = "K", test = T)$K,
                             family = PD.fam[i, "family"],
                             Response = traits[j])
    }, error = function(e) { sigK[[i]] <- NA })
  }
