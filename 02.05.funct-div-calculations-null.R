...
for(i in 1:length(spec)) {
  ...
  FDis.MULTI[[i]] <- FD.out$FDis.MULTI %>% as.data.frame()
  colnames(FDis.MULTI[[i]]) <- paste0(spec[i])
  RQE.MULTI[[i]] <- FD.out$RQE.MULTI %>% as.data.frame()
  colnames(RQE.MULTI[[i]]) <- paste0(spec[i])
}
...
FDis.MULTI.null <- do.call(cbind, FDis.MULTI)
RQE.MULTI.null <- do.call(cbind, RQE.MULTI)

list.out <- list(FDis.SLA.null,
                 RQE.SLA.null,
                 FDis.HEIGHT.null,
                 RQE.HEIGHT.null, 
                 FDis.ROOT.null,
...
names(list.out) <- c("FDis.SLA.null",
                     "RQE.SLA.null",
                     "FDis.HEIGHT.null",
                     "RQE.HEIGHT.null", 
                     "FDis.ROOT.null",
