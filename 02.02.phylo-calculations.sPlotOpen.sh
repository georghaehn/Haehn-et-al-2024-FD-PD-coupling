...
#SBATCH --job-name=PD-calculation-sPlotOpen
...
output_dir=/work/$USER/PD-calculation-sPlotOpen
mkdir -p "$output_dir"
...
Rscript --vanilla $HOME/sPlot-master-thesis/01.scripts/02.02.phylo-calculations.sPlotOpen.R \
  "$input_dir" \
  "$output" \
  "$start" \
  "$end"
